    ...
    if (items.length) {
        ...
        items.forEach(item => {
            ...
            if ( !hasOwnProp.call(item, 'longname') ) {
                itemsNav += `<li class="nav-item">${linktoFn('', item.name, 'nav-link').replace(/\.html/, '')}</li>`;
            }
                ...
                itemsNav += `<li class="nav-item">${linktoFn(item.longname, displayName.replace(/\b(module|event):/g, ''), 'nav-link').replace(/\.html/, '')}</li>`;
        ...
        if (itemsNav !== '') {
            nav += `
                <li class="nav-item">
                    <a href="#" class="nav-link collapse-toggle collapsed" data-bs-toggle="collapse" data-bs-target="#${id}" aria-expanded="false" aria-controls="${id}">
                        <i class="bi-chevron-right me-1"></i>
                        <span class="nav-link-name">${itemHeading}</span>
                    </a>
                    <div class="collapse" id="${id}" data-role="collapse">
                        <ul class="nav flex-column">${itemsNav}</ul>
                    </div>
                </li>
            `;
        }
...
function linktoExternal(longName, name) {
    return linkto(longName, name.replace(/(^"|"$)/g, ''), 'nav-link');
}
    ...
    let nav = `<ul class="nav flex-column">
            <li class="nav-item">
                <a href="index" class="nav-link">
                    ...
                    <span class="nav-link-name">Inicio</span></a>
                </li>`;
    const seen = {};
    const seenTutorials = {};
    const gid = uniqueId("nav-items");
    ...
    if (members.globals.length) {
        ...
        members.globals.forEach(({kind, longname, name}) => {
            if ( kind !== 'typedef' && !hasOwnProp.call(seen, longname) ) {
                globalNav += `<li class="nav-item">${linkto(longname, name, "nav-link")}</li>`;
            }
            seen[longname] = true;
        });
        ...
        if (!globalNav) {
            ...
            nav += `<li class="nav-item">${linkto('global', 'Global', "nav-link")}</li>`;
        }
        else {
             nav += `
                <li class="nav-item">
                    <a href="#" class="nav-link collapse-toggle collapsed" data-bs-toggle="collapse" data-bs-target="#${gid}" aria-expanded="false" aria-controls="${gid}">
                        <i class="bi-chevron-right me-1"></i>
                        <span class="nav-link-name">Global</span>
                    </a>
                    <div class="collapse" id="${gid}">
                        <ul class="nav flex-column">${globalNav}</ul>
                    </div>
                </li>
            `;
        }
    ...
    data().each(doclet => {
        ...
        if (doclet.examples) {
            doclet.examples = doclet.examples.map(example => {
                ...
                if (code.match(/^\s*<lang>([\s\S]+?)<\/lang>(\s*[\n\r])([\s\S]+)$/i)) {
                    lang = RegExp.$1;
                    code = RegExp.$3;
                }
    ...
    generate('Home',
        packages.concat(
            [{
                kind: 'mainpage',
                readme: opts.readme,
                longname: (opts.mainpagetitle) ? opts.mainpagetitle : 'Inicio'
            }]
