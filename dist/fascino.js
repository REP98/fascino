...
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("fascino", [], factory);
	else if(typeof exports === 'object')
		exports["fascino"] = factory();
	else
		root["fascino"] = factory();
})(self, () => {
 ...
 */
function Utils_toString(arg) {
  return !empty(arg) && Utils_isFunction(arg.toString) ? arg.toString() : false;
}
/**
...
function isVisible(el) {
  ...
  while (elem && elem !== Body && elem !== HTML) {
    ...
    if (Utils_toString(property.opacity) === "0") {
      return false;
    }
...
;// CONCATENATED MODULE: ./src/Classes/Data.js
...
;// CONCATENATED MODULE: ./src/Classes/Selector.js
 ...
 * @type {Array}
 */
var core_ListEvents = ['blur', 'focus', 'resize', 'scroll', 'click', 'dblclick', 'mousedown', 'mouseup', 'mousemove', 'mouseover', 'mouseout', 'mouseenter', 'mouseleave', 'change', 'select', 'submit', 'keydown', 'keypress', 'keyup', 'contextmenu', 'touchstart', 'touchend', 'touchmove', 'touchcancel'];
/**
 * @memberOf Fascino
 * @private
 * @const {Number}
...
var Fascino = /*#__PURE__*/function (_Selector) {
  ...
  return _createClass(Fascino, [{
    ...
    value: function filter(fn) {
      ...
      return extend(Utils_merge(w().Elem, [].filter.call(this.Elem, fn)), {
        _prevObj: this
      });
    }
    /**
    ...
    value: function find(sel) {
      ...
      return rEl.length === 0 ? [] : rEl.length === 1 ? w(rEl[0]) : Utils_merge(w().Elem, rEl);
    }
    /**
    ...
    value: function parents(sel) {
      ...
      this.each(function (el) {
        ...
        while (ancestor && ancestor.nodeType === Node.ELEMENT_NODE && ancestor.nodeType !== core_NODETEXT) {
    ...
    value: function attr() {
      ...
      if (args.length === 1) {
        ...
        } else if (Utils_isFunction(args[0])) {
          return this.each(function (el) {
            if (el.hasAttributes()) {
              var a = el.attributes;
    ...
    value: function fire(name, data) {
      ...
      if (core_ListEvents.indexOf(_name) > -1) {
        this.Elem[0][_name].call(this.Elem[0]);
      }
 ...
 * @return {Fascino}
 */
core_ListEvents.forEach(function (n) {
...
var U = Utils.Utils,
  Fascino_template = Utils.template,
  C = Utils.Colors;
...
//# sourceMappingURL=fascino.js.map